...
@Meyit47.on(events.NewMessage(pattern="^/start$"))
async def start(event):
  if event.is_private:
    async for usr in Meyit47.iter_participants(event.chat_id):
  ...
  if event.is_group:
    return await Meyit47.send_message(event.chat_id, f"**Beni Grubuna Aldığın için Teşekkürler ✨**")

# Başlanğıc Button
@Meyit47.on(events.callbackquery.CallbackQuery(data="start"))
...
# Maho aga
@Meyit47.on(events.callbackquery.CallbackQuery(data="komutlar"))
async def handler(event):
